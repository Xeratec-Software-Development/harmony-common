# internal C2 and C3 deploy workflow for dynamic-discover-client and harmony-MSH artifacts

This workflow is used to build the internal-test C2 and C3 versions of the Harmony application.

After build, the workflow deploys the following artifacts to the C2 and C3 servers:
...
The workflow is triggered by a push to the `harmony-xeratec-internal-c2-and-c3` branch of the `harmony-common` repository.

inputs required are branch names for the following repositories:
...
Secrets required in the XeratecSoftwareDevelopment/harmony-common "internal" environment:
